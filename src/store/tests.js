import {
  getDayOfEpoch,
  buildRandomizedValuesQueue,
  createMergeObjectArrayByProperty,
} from '../services/Utils';
import {
  getOutstandingWords,
...
export const reducer = (state = defaultState, { type, payload }) => {
  ...
  if (type === TEST_SETTESTWORDS)
    return { ...state, ...payload, stage: 0, index: 0 };
  if (type === TEST_SETTESTTYPE) return { ...state, ...payload };
  if (type === TEST_COMPLETETEST)
    return { ...state, ...payload, isComplete: true };
...
const setTestType = (type) => (dispatch, getState) => {
  ...
  const {
    words: { words },
    settings: { testingWordLimit },
  } = getState();

  const testWords = (
    type === TEST_TYPECURRENT
      ? getCurrentPracticeWords(words)
      : getOutstandingWords(words, getDayOfEpoch())
  ).slice(0, testingWordLimit);
  const queue = getRandomQueue(testWords);
  const scores = new Array(queue.length).fill(0);

  dispatch({
    type: TEST_SETTESTTYPE,
    payload: { type, testWords, queue, stage, index, scores },
  });
  return queue.length > 0;
};
const submitAnswer = (correct) => (dispatch, getState) => {
  let {
    test: { index, queue, scores, stage, testWords },
  } = getState();
  const score = correct === null ? 0 : correct ? 1 : -1;
  scores = [
    ...scores.slice(0, index),
    { id: queue[index].id, score, aspect: stage },

...
export const reducer = (state = defaultState, { type, payload }) => {
  ...
  if (type === WORDS_ADVANCESOUND) {
    ...
    if (payload === -1)
      return {
        ...state,
        currentIndex: (state.currentIndex + length - 1) % length,
        currentStage: 0,
      };
    return {
      ...state,
...
const seedPractice = (words, limit) => (dispatch) =>
  dispatch({ type: WORDS_RESEED, payload: { words, limit } });
const advancePractice = (direction) => (dispatch) =>
  dispatch({ type: WORDS_ADVANCE, payload: direction });
const nudgePractice = () => (dispatch, getState) =>
  dispatch({
    type: getState().settings.shouldJustAdvance ? WORDS_ADVANCE : WORDS_NUDGE,
  });
const closePractice = () => (dispatch) =>
  dispatch({ type: WORDS_CLOSEPRACTICE });
const advanceSound = (direction) => (dispatch) =>
  dispatch({ type: WORDS_ADVANCESOUND, payload: direction });
const nudgeSound = () => (dispatch) => dispatch({ type: WORDS_NUDGESOUND });
const registerPracticeEnd = (type, count) => (dispatch) =>
  dispatch({ type: WORDS_REGISTERPRACTICEEND, payload: { type, count } });

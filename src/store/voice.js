...
export const reducer = (state = defaultState, action) => {
  if (action.type === VOICE_SETVOICES) {
    const { thaiVoices, englishVoices, thaiVoice, englishVoice, rate } =
      action.payload;
    return {
      ...state,
      thaiVoices,
...
export const operations = {
  ...
  setEnglishVoice: (voice) => (dispatch) =>
    dispatch({ type: VOICE_SETENGLISHVOICE, payload: voice }),
  setThaiVoice: (voice) => (dispatch) =>
    dispatch({ type: VOICE_SETTHAIVOICE, payload: voice }),
  setRate: (rate) => (dispatch) =>
    dispatch({ type: VOICE_SETRATE, payload: rate }),
};

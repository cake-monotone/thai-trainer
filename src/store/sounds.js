...
export const reducer = (state = defaultState, { type, payload }) => {
  if (type === SOUNDS_LOADVOWELS && state.vowelsLoaded === false) {
    let vowels = payload
      .map(({ vowel, examples, pronunciation }) => {
  ...
  if (type === SOUNDS_LOADCONFUSION && state.confusionLoaded === false) {
    ...
    return {
      ...
      confusions,
      consonantKeys,
      confusionLoaded: true,
      visibleConfusion: 0,
    };
  }

  if (type === SOUNDS_SHOWVOWEL) return { ...state, visibleVowel: payload };
  if (type === SOUNDS_SHOWCONSONANTSBYKEY)
    return { ...state, visibleConsonantKey: payload };
  if (type === SOUNDS_SHOWCONFUSIONBYINDEX)
    return { ...state, visibleConfusion: payload };
...
const makeMoveVowel = (direction) => (currentVowel) => (dispatch, getState) => {
  ...
  const currentIndex = vowels
    .map(({ vowel }) => vowel)
    .indexOf(currentVowel.vowel);
...
const showVowel = (vowel) => (dispatch) =>
  dispatch({ type: SOUNDS_SHOWVOWEL, payload: vowel });
...
const showConsonantsByKey = (key) => (dispatch) =>
  dispatch({ type: SOUNDS_SHOWCONSONANTSBYKEY, payload: key });
const showConfusionByIndex = (index) => (dispatch) =>
  dispatch({ type: SOUNDS_SHOWCONFUSIONBYINDEX, payload: index });

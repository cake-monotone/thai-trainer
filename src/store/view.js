...
export const reducer = (state = defaultState, { type, payload }) => {
  ...
  if (type === VIEW_CHANGEPREVIEWFILTER) {
    let previewFilter = state.previewFilter.filter(
      (status) => status !== payload,
    );
    if (previewFilter.length === state.previewFilter.length)
      previewFilter.push(payload);

    return { ...state, previewFilter };
  }
  if (type === VIEW_TOGGLEHINT)
    return { ...state, hintVisible: !state.hintVisible };
  if (type === VIEW_APPLICATIONREADY)
    return { ...state, applicationReady: true };

  return state;
};

const setApplicationReady = () => (dispatch) =>
  dispatch({ type: VIEW_APPLICATIONREADY });
const changeSubview = (subview) => (dispatch) =>
  dispatch({ type: VIEW_CHANGESUBVIEW, payload: subview });
const togglePreviewFilterStatus = (status) => (dispatch) =>
  dispatch({ type: VIEW_CHANGEPREVIEWFILTER, payload: status });

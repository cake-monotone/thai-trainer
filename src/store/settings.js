...
export const SETTINGS_SETPRACTICEDISPLAYORDER =
  'settings/setpracticedisplayorder';
...
const changePronunciationType =
  (pronunciationType = null) =>
  (dispatch) => {
    dispatch({
      type: SETTINGS_SETPRONUNCIATIONTYPE,
      payload: pronunciationType,
    });
    saveSettings({ pronunciationType });
  };
const changePracticeDisplayOrder =
  ({ practiceOrder, practiceAllAtOnce }) =>
  (dispatch) => {
    if (practiceOrder) {
      dispatch({
        type: SETTINGS_SETPRACTICEDISPLAYORDER,
        payload: { practiceOrder },
      });
      saveSettings({ practiceOrder });
    } else {
      dispatch({
        type: SETTINGS_TOGGLEALLATONCE,
        payload: { practiceAllAtOnce },
      });
...
const toggleCharacterClasses = () => (dispatch, getState) => {
  ...
  dispatch({
    type: SETTINGS_TOGGLECHARACTERCLASSES,
    payload: { showCharacterClasses },
  });
...
const toggleResetProgress = () => (dispatch, getState) => {
  ...
  dispatch({
    type: SETTINGS_TOGGLERESETPROGRESS,
    payload: { resetProgressVisible },
  });
};
const resetProgress = () => async (dispatch) => {
  await resetProgressData();
  dispatch({
    type: SETTINGS_RESETPROGRESS,
    payload: { resetProgressVisible: false },
  });
};

  ...
  return (...args) => {
    let match = cache.find(
      ({ args: cachedArgs }) =>
        ...
        cachedArgs.every((entryArg, index) => entryArg === args[index]),
    );
...
export const makeExclude = (equality) => (arr1, arr2) =>
  arr1.filter((v1) => arr2.some((v2) => equality(v1, v2)) === false);
...
const rxThaiNonFullWidthCharacters =
  /[^กขฃคฅฆงจฉชซฌญฎฏฐฑฒณดตถทธนบปผฝพฟภมยรฤลฦวศษสหฬอฮฯะาำเแโใไๅๆ๏๐๑๒๓๔๕๖๗๘๙๚๛a-z ]/;
const getFullWidthCharacters = (word) =>
  word.replace(rxThaiNonFullWidthCharacters, '');
...
export const sortBySimilarity = (target, property, words) => {
  ...
  return words
    ...
    .map((word) => ({
      word,
      similarity: Levenshtein.get(
        needle,
        getFullWidthCharacters(word[property]),
      ),
    }))
    ...
    .filter(
      (word, index, arr) =>
        arr.findIndex((otherWord) => word[property] === otherWord[property]) ===
        index,
    );
};
...
export const arraysAreSimplyEqual = (a, b) =>
...
export const createMergeObjectArrayByProperty = (prop) => (a, b) => {
  return a.concat(b).reduce((carry, item, index, arr) => {
    ...
    if (firstMatch !== index)
      return [
        ...carry.slice(0, firstMatch),
        item,
        ...carry.slice(firstMatch + 1),
      ];
...
export const makeUniqueRandomSamplingIncludingValue = (allValues, length) => {
  ...
  return (requiredValue) => {
    let availableValues = shuffle(
      uniqueValues.filter((value) => value !== requiredValue),
    );
...
export const makeFilterByProps = (items) => (props) =>
  items.filter((item) =>
    Object.keys(props).every((prop) =>
      props[prop].some(
        (propsProp) =>
          item[prop] === propsProp ||
          (propsProp === null && item[prop] === undefined),
      ),
    ),
  );

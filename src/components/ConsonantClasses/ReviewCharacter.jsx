...
const getCharacterClass = memoize(
  (highlightedCharacters, character) =>
    highlightedCharacters.length === 0 ||
    highlightedCharacters.some((c) => c === character)
      ? 'highlighted'
      : null,
  100,
);
...
const ReviewCharacter = ({ character, highlight, highlightedCharacters }) => {
  ...
  return (
    <div
      className={`thai-font ${className}`}
      onMouseOver={() => highlight(character)}

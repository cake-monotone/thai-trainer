...
class Drill extends Component {
  ...
  reseed() {
    ...
    do {
      queue = makeQueue(availableCharacters);
    } while (queue[0] === currentCharacter);
  ...
  async checkAnswer(toneClass) {
    ...
    const correct =
      toneClass === TONE_CLASS_LOW && type === 'all'
        ? low
        : toneClass === TONE_CLASS_MID
          ? mid
          : toneClass === TONE_CLASS_HIGH
            ? high
            : null;
    ...
    const getAnswer = (index) => {
      ...
      return (
        <h3
          key={index}
          className={className}
          onClick={() => this.checkAnswer(tones[index])}
        >
          {labels[index]}
        </h3>
      );

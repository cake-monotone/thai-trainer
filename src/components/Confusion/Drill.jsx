...
import {
  makeUniqueRandomSamplingIncludingValue,
...
class Drill extends PureComponent {
  constructor(...args) {
    ...
    const consonantsToDrill = confusions[visibleConfusion].map((char) => ({
      ...
      pronunciation:
        consonants[char][
          pronunciationType === PRONUNCIATIONTYPE_PAIBOON ? 3 : 2
        ],
    }));

    getPossibleAnswers = makeUniqueRandomSamplingIncludingValue(
      consonantsToDrill.map((consonant) => consonant.pronunciation),
      4
    );
  ...
  advance(direction = 1) {
    ...
    const currentConsonant =
      queue[(currentIndex + queue.length) % queue.length];
  ...
  render() {
    ...
    const answers = possibleAnswers.map((possibleAnswer, index) => (
      <DrillAnswer
        key={possibleAnswer}
        correctAnswer={correctAnswer}
        mistakes={mistakes}
        index={index}
        onSelect={() => this.selectAnswer(index)}
    ...
    return (
      <div className="drill">
        <Hint {...this.props} title="Instructions">
          ...
          <p>
            You will advance automatically when you make the correct choice.
          </p>
        </Hint>

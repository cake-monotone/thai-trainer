...
class Drill extends PureComponent {
  ...
  reseed(currentQueue, stage) {
    ...
    do {
      queue = randomize(tones).map((item) => ({
        item,
        word: item.examples[Math.floor(Math.random() * item.examples.length)],
        tone: getTone(item),
      }));
    } while (
      currentQueue &&
      queue.tone === currentQueue[currentQueue.length - 1].tone
    );
  ...
  async checkAnswer(answer) {
    ...
    let { queue, correctItem, hiddenItems, index, label, mistakeMake, stage } =
      this.state;
    let delayTime = 300;
  ...
  onKey({ code }) {
    ...
    else if (code === 'Enter' || code === 'Return' || code === 'Space')
      this.checkAnswer(TONE_MID);
  }

  render() {
    const { queue, index, hiddenItems, correctItem, label, mistakeMake } =
      this.state;
    ...
    const getAnswer = (key, tone, label) => {
      ...
      return (
        <h3 className={className} onClick={() => this.checkAnswer(tone)}>
          {label}
        </h3>
      );
    };

...
const getConsonants = memoize((consonants, visibleConsonants) =>
...
const ConsonantList = ({
  ...
  const items = getConsonants(
    consonants,
    consonantKeys[visibleConsonantKey],
  ).map(({ character, consonant: [, , ipa, paiboon, thai] }) => (
    <div key={character}>
      <span>
        <ColorizedThai word={{ thai: character }} />
      </span>
      <span>{pronunciationType === PRONUNCIATIONTYPE_IPA ? ipa : paiboon}</span>
      <span className="thai-font">{thai}</span>
      <span>
        <PlayButton word={{ thai }} />
      </span>
    </div>
  ));

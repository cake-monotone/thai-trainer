...
const keyToProperty = (key, pronunciationType) => {
  if (key === 'pronunciation')
    return pronunciationType === 'IPA' ? 'ipa' : 'paiboon';
  return key;
};
...
class Challenge extends PureComponent {
  constructor(...args) {
    ...
    let property = keyToProperty(
      stageTestTargets[stage].answer,
      pronunciationType,
    );

    const answers = shuffle([
      ...sortBySimilarity(queue[index], property, words).slice(0, 8),
      queue[index],
    ]);
    this.state = {
      property,
  ...
  render() {
    ...
    return (
      <div className="challenge">
        <Question
          word={correctAnswer}
          property={keyToProperty(
            stageTestTargets[stage].question,
            pronunciationType,
          )}
        />

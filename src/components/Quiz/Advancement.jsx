...
class Advancement extends Component {
  ...
  render() {
    ...
    if (isAnswered && isLastQuestion)
      return (
        <div className="advancement">
          <button
            className={isCorrect ? 'default success' : 'default fail'}
            ref={this.defaultButtonRef}
            onClick={() => submitAnswer(isCorrect)}
          >
            View Results
          </button>
        </div>
    ...
    return (
      <div className="advancement">
        <button className="default" onClick={() => submitAnswer(null)}>
          Not sure / Skip
        </button>
      </div>
    );

...
const TestResult = ({ saveTest, testWords, scores }) => {
  const items = testWords.map((testWord) => {
    const aspectScores = scores.reduce(
      (aspectScores, { id, score, aspect }) => {
    ...
    return (
      <Item key={testWord.id} word={testWord} aspectScores={aspectScores} />
    );
  });
  ...
  return (
    <div className="test-results">
      <h1>Results</h1>
      <section>
        <button className="save-button" onClick={() => saveTest(scores)}>
          Save results
        </button>
      </section>

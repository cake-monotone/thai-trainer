...
export default class CardContainer extends Component {
  ...
  onKey({ code }) {
    ...
    if (
      !practiceAllAtOnce &&
      (code === 'Space' || code === 'ArrowDown' || code === 'Enter')
    )
      nudgePractice();
  ...
  render() {
    ...
    if (!word) return null;
    if (practiceAllAtOnce || practiceOrder[currentStage] === 'pronunciation')
      setTimeout(() => say(LANGUAGE_THAI, word), 300);

    const card = (
      <CSSTransition key={word.id} timeout={500} classNames="card">
        <Card
          word={queue[currentIndex]}
          stage={currentStage}
          onClick={() => nudgePractice(practiceAllAtOnce)}
    ...
    return (
      <Fragment>
        <Hint {...this.props} title="Instructions">
          ...
          <p>
            Go to settings to change the number of words to practice at once.
          </p>
        </Hint>
        <TransitionGroup className="cards">{card}</TransitionGroup>
      </Fragment>
    );
  }
}
